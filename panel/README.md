...
### フォーマット（16-bit 値 20 個 / Unit）

| インデックス | パラメータ | 範囲 | 説明 |
|------------|----------|------|------|
| **0** | **Destination Unit** | 0-15 | 宛先 Sound Unit |
| **1** | **VCO1 Phase Delta** | 0-65535 | VCO1 位相増分（panel 側で算出） |
| **2** | **VCO2 Phase Delta** | 0-65535 | VCO2 位相増分（panel 側で算出） |
| **3** | **VCO1 WaveType** | 0-3 | VCO1 波形 (Saw/Sin/Tri/Square) |
| **4** | **VCO2 WaveType** | 0-3 | VCO2 波形 (Saw/Sin/Tri/Square) |
| **5** | **VCO1 Duty** | 0-65535 | PWM デューティサイクル |
| **6** | **VCO Mix** | 0-65535 | VCO1/VCO2 ミックスバランス |
| **7** | **VCF Type** | 0-1 | フィルタタイプ (0=LPF, 1=HPF) |
| **8-9** | **VCF b0** | float(32bit) | 双二次係数 b0（上位16/下位16） |
| **10-11** | **VCF b1** | float(32bit) | 双二次係数 b1（上位16/下位16） |
| **12-13** | **VCF b2** | float(32bit) | 双二次係数 b2（上位16/下位16） |
| **14-15** | **VCF a1** | float(32bit) | 双二次係数 a1（上位16/下位16） |
| **16-17** | **VCF a2** | float(32bit) | 双二次係数 a2（上位16/下位16） |
| **18** | **VCA Gain Left** | 0-65535 | 左チャンネル ゲイン (0.0～1.0) |
| **19** | **VCA Gain Right** | 0-65535 | 右チャンネル ゲイン (0.0～1.0) |

### 送信プロトコル

- **総データ数**: 16 Unit × 20 値 = 320 ワード
